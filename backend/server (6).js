...
// ── STAFF JARS REPORT ────────────────────────────────────────────────────────
app.get("/api/staff-jars", async (req, res) => {
  try {
    const TARGET = 3000;
    const raw = await fetchSheet("staff report");

    const staff = raw.map((r, i) => {
      const name = String(getField(r, "staff name", "name", "staff") || `Staff ${i + 1}`);
      const jarsSold = Number(getField(r, "jars sold", "jars", "total jars", "sold") || 0);

      // Incentive calculation
      const base     = jarsSold * 1.5;
      const bonus500  = jarsSold >= 500  ? 500  : 0;
      const bonus1000 = jarsSold >= 1000 ? 1000 : 0;
      const bonus3000 = jarsSold >= 3000 ? 2500 : 0;
      const total = base + bonus500 + bonus1000 + bonus3000;

      return { name, jarsSold, incentive: { base, bonus500, bonus1000, bonus3000, total }, rank: 0 };
    }).filter(s => s.name && s.jarsSold >= 0);

    staff.sort((a, b) => b.jarsSold - a.jarsSold);
    staff.forEach((s, i) => s.rank = i + 1);

    res.json({ staff, target: TARGET });
  } catch (err) {
    res.status(500).json({ error: "Failed to fetch staff jars data", details: err.message });
  }
});
